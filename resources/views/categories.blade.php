...
@section('content')
	<div class="text-center text-slate-600 mb-6 py-4">
		<h1 class="text-xl md:text-2xl font-semibold">Categories List</h1>
	</div>

    <div class="flex justify-center mb-6">
		<form class="w-full lg:w-1/2 overflow-auto" action="{{ route('searchCategory') }}" method="GET">
			@csrf
			<div class="input-group relative flex flex-wrap items-stretch w-full">
				<input type="search" class="form-control relative flex-auto min-w-0 block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding 
					...
					placeholder="Search" aria-label="Search" aria-describedby="button-addon2" name="categorySearch"
					value="{{ session()->has('categorySearch') ? session('categorySearch') : '' }}">
    ...
    @if (session("success"))
        <div class="flex sm:justify-center mb-4">
            ...
            <div class="bg-white rounded-xl border border-slate-300 shadow overflow-auto h-fit">
                <div class="my-8">
                    ...
                    <div class="text-slate-400 text-sm px-8 mt-2">
                        {{ $category_list->appends(Request::all())->links() }}
                    </div>
                </div>
            </div>
        </div>
